...
(defun relation-to-sql (relation)
  (let* ((from (relation-from relation))
         ...
         (compute (relation-compute relation))
         ...
         (clauses (list
                   (mqi.sxql:make-from-clause
                    (sxql:make-sql-symbol 
                     (mito.class.table:table-name class))))))
    ;; TODO: SELECT and COMPUTE are exclusive
    (when joins
    ...
    (when select
      (let ((columns (select-columns select)))
        (setf fields (loop for column in columns
                        collect
                           (if joins
                               (rel-column-name
                                (mito.class.table:table-name class)
                                column)
                               (alexandria:make-keyword column))))))
    (when order
    ...
    (when compute
      (ecase (compute-tag compute)
        (:count (setf fields `((:as (:count :*) :__mqi_count))))
        (:maximum (let ((column (compute-maximum-column compute)))
                    (when joins
